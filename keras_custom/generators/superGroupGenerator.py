...
class SafeDirectoryIterator(SafeIterator):
    ...
    def __init__(self,
        ...
        if not classes:
            # this only calls when classes=None, which means get all classes.
            classes = []
        ...
        sup_wnids, sup_indices, sup_descriptions = load_classes(num_classes=999, df='canidae')
        
        for wnid in self.class_indices:
            # TODO: here which label we set to depends on
            # TODO: if we fine tune the VGG output or not.
            # TODO: if finetune, we could set the label to be the last index
            # TODO: if not, we could set the label to be the first dog's index.
        ...
        #exit()
        # REVIEW: the hack until this point.
        #######################################################################################
    ...
    def _get_batches_of_transformed_samples(self, index_array, wordvec_mtx):
        ...
        if self.class_mode == 'input':
            ...
            ###
            # HACK: manage superGroup labelling
            # 1. we need to make sure semantic ground truth stays the same
            # and since we use Nick's method to get the ground truth word
            # vectors, we need to get the original batch_y first.
            semantic_truth = np.dot(batch_y, wordvec_mtx)

            # 2. Now the main business which is to get the ground truth labels
            # for the groupSuper case based on `class_indices_sup` and `classes_sup`
        ...
        ###
        # the model still outputs 2 sets of outputs as usual.
        batch_y = [semantic_truth, batch_y_sup]
        ###
