...
"""
    Purpose:
    --------
        27/03/2020: the parent classes of `generator_for_continuous_master`
                    that is only used when training continuous_master model.
"""
...
class SafeDirectoryIterator(SafeIterator):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        ###### second and third input to attention factory (TEST) #####################
        # v2.
        non_trg_int = (1 - self.intensity) / 999  # always 999 for adv200 context
        prior = np.ones((batch_x.shape[0], 1000)) * non_trg_int

        prior[:, self.focus_index] = self.intensity

        # complete triple input
        intensity_this_epoch = np.ones((batch_x.shape[0], 1)) * self.intensity
