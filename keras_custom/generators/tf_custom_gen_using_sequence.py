...
class SafeIterator(Sequence):
    ...
    def _set_index_array(self):
        ...
        # print('subsample_dict', subsample_dict)
        new_index_array = []
        for class_i in subsample_dict:
            ...
            if weight == 1:
                ...
                # np.random.seed(42)  # NOTE: can't seed because we want
                # different samples each at each epoch
...
class SafeDirectoryIterator(SafeIterator):
    ...
    def __init__(self,
                 ...
                 #############################################
                 # WARNING: will pass in a list from user
                 focus_classes=['n02123045'],
                 subsample_rate=0.01
                 #############################################
                 ):
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        ###### second input ####################################################
        SIZE = 512
        # WARNING (ken): Second input size will have to be automated soon.')
        second_input = np.ones((batch_x.shape[0], SIZE), dtype=self.dtype)       # TODO: 29/02/2020, size changes based on where attention layer is
        batch_x = [batch_x, second_input]
        ########################################################################
