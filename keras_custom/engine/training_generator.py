...
######################################################################
  batch_function = _make_execution_function(
      model, mode, train_class_weight=train_class_weight, val_class_weight=val_class_weight)
######################################################################
...
evaluate_generator = functools.partial(model_iteration, mode=ModeKeys.TEST, shuffle=False)
predict_generator = functools.partial(model_iteration, mode=ModeKeys.PREDICT, shuffle=False)
#############################################################################################
...
def _get_next_batch(generator):
  ...
  try:
    generator_output = next(generator)                 # NOTE: here is becomes a normal generator... help(generator)
    # print('_get_next_batch: next(generator)')          # NOTE: very strange because when outside gen, call next(gen) returns the custom gen class
...
#########################################################################################
def _make_execution_function(model, mode, train_class_weight=None, val_class_weight=None):
  """Makes function to run one step of model execution."""
  if mode == ModeKeys.TRAIN:
      f = functools.partial(model.train_on_batch_custom, class_weight=train_class_weight)


# ----------------------------------------------------------------------------
  elif mode == ModeKeys.TEST:
      # print('### CHECK: using class_weight for ### [val] ###', val_class_weight)
      f = functools.partial(model.test_on_batch_custom, class_weight=val_class_weight)
# ----------------------------------------------------------------------------
  else:
